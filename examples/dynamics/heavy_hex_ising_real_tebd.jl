...
using ITensorNetworks: initialize_bond_tensors, vidal_itn_canonicalness, vidal_to_symmetric_gauge, vidal_gauge,
    approx_network_region, optimal_contraction_sequence, contract, symmetric_to_vidal_gauge, norm_network
...
function create_heavy_hex_grid(no_qubits::Int64)
    ...
    rung_qubits = [14, 15, 16, 17, 33, 34, 35, 36, 52, 53, 54, 55, 71, 72, 73, 74, 90, 91, 92, 93, 109, 110, 111, 112]
    qubit_pairs = [(0, 18), (4,22), (8, 26), (12, 30), (20, 39), (24, 43), (28, 47), (32, 51), (37, 56), (41, 60), (45, 64), (49, 68), (58, 77), (62, 81),
        (66, 85), (70, 89), (75, 94), (79, 98), (83, 102), (87, 106), (96, 114), (100, 118), (104, 122), (108, 126)]
...
function expect_state_SBP(o::ITensor, ψ::AbstractITensorNetwork, ψψ::AbstractITensorNetwork, mts::DataGraph)
    ...
    return contract(numerator_network; sequence = num_seq)[] / contract(denominator_network, sequence = den_seq)[]
...
function main(θh::Float64, no_trotter_steps::Int64; apply_kwargs...)
    ...
    for i in 1:no_trotter_steps
        ...
        gates = (shortened_final_layer && i == no_trotter_steps) ? RX_gates : vcat(RX_gates, RZZ_gates)
        ...
        if ((i+1) % gauge_freq) == 0 && (cur_C >= target_c)
            ...
            ψ, bond_tensors = vidal_gauge(ψ; target_canonicalness = target_c, niters = 20, cutoff = 1e-14)
        end
    end
    ...
    ψψ = norm_network(ψ)
    mag_dict = Dict(zip([v for v in vertices(ψ)], [expect_state_SBP(op("Z", s[v]), ψ, ψψ, mts) for v in vertices(ψ)]))
...
println("Simulating $no_trotter_steps Steps of the Kicked Ising Model on the Heavy Hex Layout.")
println("θh is $θh. Maxdim is $χ.")

mags = main(θh, no_trotter_steps; apply_kwargs...)

println("After $no_trotter_steps steps. Average magnetisation is "*string(mean(values(mags))))