...
@testset "Observers" begin
    ...
    @test region_observer![30, :energy] ≈ e rtol = 1.0e-6
end
...
@testset "Regression test: Arrays of Parameters" begin
    ...
    cutoff = [1.0e-10, 1.0e-10, 1.0e-12, 1.0e-12, 1.0e-12, 1.0e-12, 1.0e-14, 1.0e-14, 1.0e-14, 1.0e-14]
...
@testset "Tree DMRG" for nsites in [2]
    ...
    @testset "SVD approach" for use_qns in [false, true]
        ...
        @test inner(psi', H, psi) ≈ ITensorMPS.inner(psi2', Hline, psi2) atol = 1.0e-5
...
@testset "Tree DMRG for Fermions" for nsites in [2] #ToDo: change to [1,2] when random_ttn works with QNs
    ...
    @test inner(psi', H, psi) ≈ ITensorMPS.inner(psi2', Hline, psi2) atol = 1.0e-5
    @test e2 ≈ e_jw atol = 1.0e-5
    @test inner(psi2', Hline, psi2) ≈ e_jw atol = 1.0e-5
