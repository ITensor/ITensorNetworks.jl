...
@testset "MPS DMRG-X" for conserve_qns in (false, true)
    ...
    ψ = mps(v -> rand(rng, ["↑", "↓"]), s)
    dmrg_x_kwargs = (nsweeps = 20, normalize = true, maxdim = 20, cutoff = 1.0e-10, outputlevel = 0)
    e, ϕ = dmrg_x(H, ψ; nsites = 2, dmrg_x_kwargs...)
    @test inner(ϕ', H, ϕ) / inner(ϕ, ϕ) ≈ e
    @test inner(ψ', H, ψ) / inner(ψ, ψ) ≈ inner(ϕ', H, ϕ) / inner(ϕ, ϕ) rtol = 1.0e-1
    @test inner(H, ψ, H, ψ) ≉ inner(ψ', H, ψ)^2 rtol = 1.0e-7
    @test inner(H, ϕ, H, ϕ) ≈ inner(ϕ', H, ϕ)^2 rtol = 1.0e-7
    e, ϕ̃ = dmrg_x(H, ϕ; nsites = 1, dmrg_x_kwargs...)
    @test inner(ϕ̃', H, ϕ̃) / inner(ϕ̃, ϕ̃) ≈ e
    @test inner(ψ', H, ψ) / inner(ψ, ψ) ≈ inner(ϕ̃', H, ϕ̃) / inner(ϕ̃, ϕ̃) rtol = 1.0e-1
    @test inner(H, ϕ̃, H, ϕ̃) ≈ inner(ϕ̃', H, ϕ̃)^2 rtol = 1.0e-3
...
@testset "Tree DMRG-X" for conserve_qns in (false, true)
    ...
    @test inner(ϕ', H, ϕ) / inner(ϕ, ϕ) ≈ e
    @test inner(ψ', H, ψ) / inner(ψ, ψ) ≈ inner(ϕ', H, ϕ) / inner(ϕ, ϕ) rtol = 1.0e-1
    @test inner(H, ψ, H, ψ) ≉ inner(ψ', H, ψ)^2 rtol = 1.0e-2
    @test inner(H, ϕ, H, ϕ) ≈ inner(ϕ', H, ϕ)^2 rtol = 1.0e-7
    e, ϕ̃ = dmrg_x(H, ϕ; nsites = 1, dmrg_x_kwargs...)
    @test inner(ϕ̃', H, ϕ̃) / inner(ϕ̃, ϕ̃) ≈ e
    @test inner(ψ', H, ψ) / inner(ψ, ψ) ≈ inner(ϕ̃', H, ϕ̃) / inner(ϕ̃, ϕ̃) rtol = 1.0e-1
    @test inner(H, ϕ̃, H, ϕ̃) ≈ inner(ϕ̃', H, ϕ̃)^2 rtol = 1.0e-6
    # Sometimes broken, sometimes not
    ...
    @test inner(ϕ', H, ϕ) ≈ (dag(U_exact') * T * U_exact)[] atol = 1.0e-6
    @test abs(inner(U_dmrgx, U_exact)) ≈ 1 atol = 1.0e-6
end
end
