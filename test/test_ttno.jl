...
using ITensorNetworks: TreeTensorNetwork, contract, ortho_region, siteinds, union_all_inds
...
@testset "TTN operator Basics" begin
    ...
    @testset "Construct TTN operator from ITensor or Array" begin
        ...
        @disable_warn_order o1 = TreeTensorNetwork(O, is_isp; cutoff)
        root_vertex = only(ortho_region(o1))
        @disable_warn_order begin
            O1 = contract(o1, root_vertex)
        end
        @test norm(O - O1) < 1.0e2 * cutoff
    end
    @testset "Ortho" begin
        # TODO
    end
