...
@testset "Contract MPO" begin
    ...
    @test inner(psi, Hpsi) ≈ inner(psi', H, psi) rtol = 1.0e-5
    ...
    @test abs(inner(Hpsi_via_dmrg, Hpsi / norm(Hpsi))) ≈ 1 rtol = 1.0e-4
    ...
    @test inner(psi, Hpsi) ≈ (inner(psi', H, psi) - norm(psi)^2) rtol = 1.0e-5
    ...
    @test_broken abs(inner(Hpsi, (Hpsi_normalized) / norm(Hpsi))) ≈ 1 rtol = 1.0e-5
    ...
    @test inner(psit, Hpsi) ≈ inner(psit, H, psi) rtol = 1.0e-5
    ...
    @test inner(psit, Hpsi) ≈ inner(psit, H, psi) rtol = 1.0e-4
end
...
@testset "Contract TTN" begin
    ...
    Hpsi = apply(
        H, psi; alg = "fit", init = psi, nsweeps = 5, maxdim = [16, 32], cutoff = [1.0e-4, 1.0e-8, 1.0e-12]
    )
    @test inner(psi, Hpsi) ≈ inner(psi', H, psi) rtol = 1.0e-2
    ...
    @test inner(psit, Hpsi) ≈ inner(psit, H, psi) rtol = 1.0e-2
end
end
