...
using ITensorNetworks: ising_network, compute_message_tensors, group_partition_vertices, calculate_contraction, split_index
using Test
using Compat
using ITensors
...
@testset "belief_propagation" begin
  ...
  bp_sz = calculate_contraction(ψψ, mts, [(v,1)]; verts_tensors = ITensor[apply(op("Sz", s[v]), ψ[v])])[]/ calculate_contraction(ψψ, mts,  [(v,1)])[]
  ...
  bp_szsz = calculate_contraction(ψψ, mts, vs; verts_tensors = [ψOψ[v] for v in vs])[]/calculate_contraction(ψψ, mts, vs)[]
  ...
  rdm = calculate_contraction(ψψ, mts, [(v,2) for v in vs]; verts_tensors = ITensor[ψψsplit[vp] for vp in [(v,2) for v in vs]])
