...
function local_boltzmann_weight(
  ::Model"ising", ::Val{1}; β::Real, J::Real=1.0, sz::Bool=false
)
...
function local_boltzmann_weight(
  ::Model"ising", ::Val{2}; β::Real, J::Real=1.0, sz::Bool=false
)
  d = 2 # local dimension of the Ising local Boltzmann factor
  sₕ, sₕ′ = Index.((d, d))
  sᵥ, sᵥ′ = Index.((d, d))
...
function mpo_itensor(
  m::Model, pair_sₕ::Pair{<:Index,<:Index}, pair_sᵥ::Pair{<:Index,<:Index}; kwargs...
)
  sₕ, sₕ′ = pair_sₕ
  sᵥ, sᵥ′ = pair_sᵥ
