...
using NamedGraphs.GraphsExtensions: forest_cover, connected_components, add_edges, add_edge, add_vertices, rem_edges, decorate_graph_edges, rename_vertices

using ITensors: OpSum, siteinds, sites, inds, combiner
using ITensors.NDTensors: array
using ITensorNetworks: AbstractITensorNetwork, ITensorNetwork, IndsNetwork, underlying_graph, ttn, maxlinkdim
...
function connect_forests(s::IndsNetwork)
    ...
    for f in fs
        ...
        dummy_es = NamedEdge[NamedEdge(first(vertices(c_f[i])) => first(vertices(c_f[i+1]))) for i in 1:(length(c_f) - 1)]
...
function opsum_to_tno(s::IndsNetwork, H::OpSum; cutoff::Float64 = 1e-14, insert_dummy_inds = false)
