...
default_krylov_kwargs() = (; tol = 1e-14, krylovdim = 3, maxiter = 1, verbosity = 0, eager = false, ishermitian = true)
...
function get_new_state(∂ψOψ_bpc_∂rs::Vector, inv_sqrt_mts, state::ITensor; sequences = ["automatic" for i in length(∂ψOψ_bpc_∂rs)])
...
function bp_eigsolve_updater(init::ITensor, ∂ψOψ_bpc_∂rs::Vector, sqrt_mts, inv_sqrt_mts; krylov_kwargs = default_krylov_kwargs())
    ...
    get_new_state_ = state -> get_new_state(∂ψOψ_bpc_∂rs, inv_sqrt_mts, state; sequences)
    howmany = 1
