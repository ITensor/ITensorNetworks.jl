...
function _npartitions(g::AbstractGraph, npartitions::Integer, nvertices_per_partition::Nothing)
  return npartitions
end

function _npartitions(g::AbstractGraph, npartitions::Nothing, nvertices_per_partition::Integer)
...
function _npartitions(g::AbstractGraph, npartitions::Nothing, nvertices_per_partition::Nothing)
  return error("Must specify either `npartitions` or `nvertices_per_partition`")
end

function partition_vertices(g::Graph; npartitions=nothing,  nvertices_per_partition=nothing, backend=current_partitioning_backend(), kwargs...)
  return partition_vertices(Backend(backend), g, _npartitions(g, npartitions, nvertices_per_partition); kwargs...)
end

function partition_vertices(g::NamedGraph; npartitions=nothing,  nvertices_per_partition=nothing, kwargs...)
  vertex_partitions = partition_vertices(parent_graph(g); npartitions, nvertices_per_partition, kwargs...)
  ...
  return [parent_vertices_to_vertices(g, vertex_partition) for vertex_partition in vertex_partitions]
end

function partition_vertices(g::AbstractDataGraph; npartitions=nothing,  nvertices_per_partition=nothing, kwargs...)
  return partition_vertices(underlying_graph(g); npartitions, nvertices_per_partition, kwargs...)
end
...
graph `subgraph_vertices`, return a DataGraph storing the subgraphs
on the vertices of the graph and with edges denoting
which subgraphs of the original graph have edges, along with
edge data storing the edges original edges that were connecting the subgraphs
and the edge data if relavent.
"""
function partition(g::AbstractGraph, subgraph_vertices)
  dg_subgraphs = DataGraph(NamedGraph(eachindex(subgraph_vertices)), subgraphs(g, Dictionary(subgraph_vertices)))
  for e in edges(g)
    s1 = findfirst_on_vertices(subgraph -> src(e) ∈ vertices(subgraph), dg_subgraphs)
    s2 = findfirst_on_vertices(subgraph -> dst(e) ∈ vertices(subgraph), dg_subgraphs)
    if (!has_edge(dg_subgraphs, s1, s2) && s1 ≠ s2)
      add_edge!(dg_subgraphs, s1, s2)
      dg_subgraphs[s1 => s2] = Dictionary([:edges, :edge_data], [Vector{edgetype(g)}(), Dictionary{edgetype(g),edge_data_type(g)}()])
    end
    if has_edge(dg_subgraphs, s1, s2)
      push!(dg_subgraphs[s1 => s2][:edges], e)
      if isassigned(g, e)
        set!(dg_subgraphs[s1 => s2][:edge_data], e, g[e])
      end
    end
  end
  return dg_subgraphs
...
Given a graph g, partition the graph into 'npartitions' partitions. Try to keep all subgraphs the same size and minimise edges cut between them
...
"""
function partition(g::AbstractGraph; npartitions=nothing,  nvertices_per_partition=nothing, kwargs...)
  return partition(g, partition_vertices(g; npartitions, nvertices_per_partition, kwargs...))
end
