function contract(
  ...
  init=random_ttn(flatten_external_indsnetwork(first(tn1s), first(tn2s)); link_space=trivial_space(first(tn1s))),  ##not generic yet
  nsweeps=1,
  ...
  any(ns .!= n) && throw(
    DimensionMismatch("Number of sites in different operators ($n) do not match")
  )
...
function contract(tn1s::Vector{<:AbstractTTN}, tn2s::Vector{<:AbstractTTN}; alg="fit", kwargs...)
...
function apply(tn1s::Vector{<:AbstractTTN}, tn2s::Vector{<:AbstractTTN}; alg="fit", kwargs...)
